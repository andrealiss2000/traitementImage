...
public class TraitementImagesUtils {
	...
	public static Image filtreMedian(Image img) {
		...
		private static List<Double> convertToHSV(int r, int g, int b) { 
			...
			if(g >= b) {
				...
				System.out.println("EROROTGOHKRETHGORHOHRTHTRHORHOREHTROIHEIOGHUTGUIHG");
			}
			      ...
			      while (myReader.hasNextLine()) {
			        ...
			        if(data!=null) {
			        	 ...
			        	 for(int i=1; i<data.length; i++) {
			        		 //System.out.println("tailllelelelelelelele : " + data[0]);
							 ...
							 double dist = getDistance(queryHistogram, histogram);
							 //System.out.println("la distance de "+ data[0] + " : " + dist);
							 
							 if(!distances.containsValue(fileName)) {
	...
	private static double getDistance(double[][] histogramQuery, double[][] histogramImage) {
		...
		if(histogramImage.length == histogramQuery.length) {
			...
			double resultat = distanceR+distanceG+distanceB;
			
	        DecimalFormat df = new DecimalFormat("#.##########"); 
	        String rounded = df.format(resultat); 
	        double resultC = Double.parseDouble(rounded.replace(',', '.'));
