...
public class TraitementImagesUtils {
	...
	public static Image filtreMedian(Image img) {
		...
		public static double[][] getHistogramHSV(Image image) {
			...
			for(int x=0; x<img.getXDim(); x++) {
				for(int y=0; y<img.getYDim();y++) {
					...
					int s = (int) Math.round(hsvList.get(1)*100); // 958 puis diviser par 1000 pour avoir une valeur entre 0 et 1
					int v = (int) Math.round(hsvList.get(2)*100); // 156 puis diviser par 1000 pour avoir une valeur entre 0 et 1 
		...
		private static List<Double> convertToHSV(int r, int g, int b) { 
			...
			double max = Math.max(rHSV, Math.max(gHSV, bHSV));
			double min = Math.min(rHSV, Math.max(gHSV, bHSV));
			 
			double h = 0.0;
			double s = 0.0;
			double v = 0.0;
			
			double sqrt = Math.pow(rHSV, 2) + Math.pow(gHSV, 2) + Math.pow(bHSV, 2) - rHSV*gHSV - rHSV*bHSV - gHSV*bHSV; 
			double acos = Math.acos((r-(g/2)-(b/2))/Math.sqrt(sqrt)); 
		...
		public static double[][] normalise(double[][] histogram, int nbPixels) {
			double[][] newHistogram = new double[SIZE_HISTO][3];
			System.out.println("nb pixl : " + nbPixels); 
			for(int i=0; i< histogram.length; i++) {
		...
		public static Map getSimilarImages(Image query, String type) {
